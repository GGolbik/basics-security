...
public class SecurityToolsController : ControllerBase
{
    ...
    public IActionResult BuildCert([FromForm(Name = "config")] IFormFile configFile, [FromForm(Name = "files[]")] List<IFormFile> files)
    {
        var config = this.GetConfig<ConfigCert>(configFile);
        var configFiles = new HashSet<X509File>();
        FindAllFiles(configFiles, config);
        ReplaceFilename(configFiles, files);

        // build
        var result = _service.Build(config);

        // create files
        var resultFiles = new Dictionary<string, byte[]>
        {
            { "input.conf.json", Encoding.UTF8.GetBytes(JsonSerializer.Serialize(config, Program.JsonOptions)) },
            { "output.conf.json", Encoding.UTF8.GetBytes(JsonSerializer.Serialize(result, Program.JsonOptions)) },
    ...
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest, "application/json")]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError, "application/json")]
    public IActionResult BuildCsr([FromForm(Name = "config")] IFormFile configFile, [FromForm(Name = "files[]")] List<IFormFile> files)
    {
        var config = this.GetConfig<ConfigCsr>(configFile);
        var configFiles = new HashSet<X509File>();
        FindAllFiles(configFiles, config);
        ReplaceFilename(configFiles, files);
    ...
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest, "application/json")]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError, "application/json")]
    public IActionResult BuildCrl([FromForm(Name = "config")] IFormFile configFile, [FromForm(Name = "files[]")] List<IFormFile> files)
    {
        var config = this.GetConfig<ConfigCrl>(configFile);
        var configFiles = new HashSet<X509File>();
        FindAllFiles(configFiles, config);
        ReplaceFilename(configFiles, files);
    ...
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest, "application/json")]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError, "application/json")]
    public IActionResult BuildKeyPair([FromForm(Name = "config")] IFormFile configFile, [FromForm(Name = "files[]")] List<IFormFile> files)
    {
        var config = this.GetConfig<ConfigKeyPair>(configFile);
        var configFiles = new HashSet<X509File>();
        FindAllFiles(configFiles, config);
        ReplaceFilename(configFiles, files);
    ...
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest, "application/json")]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError, "application/json")]
    public IActionResult Transform([FromForm(Name = "config")] IFormFile configFile, [FromForm(Name = "files[]")] List<IFormFile> files)
    {
        var config = this.GetConfig<ConfigTransform>(configFile);
        var configFiles = new HashSet<X509File>();
        FindAllFiles(configFiles, config);
        ReplaceFilename(configFiles, files);
    ...
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest, "application/json")]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError, "application/json")]
    public IActionResult Print([FromForm(Name = "files[]")] List<IFormFile> files)
    {
        var reqConfig = new ConfigTransform
        {
            Mode = TransformMode.Print,
    ...
    private T GetConfig<T>(IFormFile configFile) {
        ...
        {
            var config = JsonSerializer.Deserialize<T>(stream, Program.JsonOptions);
            if (config == null)
            {
