...
public class TerminalApp : IDisposable
{
    ...
    private ExitCode Start(CertOptions certOptions)
    {
        ...
        if (certOptions.OutputFile != null)
        {
            ...
            {
                { "input.conf.json", Encoding.UTF8.GetBytes(JsonSerializer.Serialize(request, _options.GetJsonSerializerOptions())) },
                { "output.conf.json", Encoding.UTF8.GetBytes(JsonSerializer.Serialize(config, _options.GetJsonSerializerOptions())) },
            };
